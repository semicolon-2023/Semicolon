...
async function fetchData() {
  try {
    const response = await fetch('https://semicolon23.onrender.com/dog');
...
topics.addEventListener('click', async (event) => {
  if (event.target.parentNode.classList.contains('card')) {
    ...
    await fetch('https://semicolon23.onrender.com/showPost', {
      method: "POST",
      headers: {
    ...
    window.location.href = 'https://semicolon23.onrender.com/show';
  }
})